...
const name = n => `Person ${String(n).padStart(2, '0')}`;
const summaries = ['Exploring ideas together · design and making', 'Weekend walks and city notes · interest group', 'Sharing reading notes · a long correspondence', 'Open-source work and technical chat · community'];
const people = Array.from({ length: 24 }, (_, i) => ({ id: id(i + 1), display_name: name(i + 1), profile_type: 'person', summary: summaries[i % 4] }));
const tags = ['Creative partners', 'Reading group', 'Open source', 'Weekend walks', 'Stay in touch', 'New friends'].map((name, i) => ({ id: id(500 + i), name, people: 4, sources: ['manual'] }));
...
const timeline = [
  ['wechat', 'A quieter weekend', 'Weekends for walking, reading, and new ideas. This copy was written only for the public showcase.'],
  ['instagram', 'Collecting ordinary color', 'Small notes about light, color, and daily looking. Demo posts do not map to any real account.'],
  ['linkedin', 'Turning talk into work', 'A community project share. It is good to watch an idea move from discussion into practice.'],
  ['wechat', 'Before the next meeting', 'A short list for the next reading group. Looking forward to trading more interesting ideas.'],
...
const server = http.createServer((req, res) => {
  ...
  switch (url.pathname) {
    ...
    case '/api/v1/metrics/people-overview': body = { people: 128, groups: 12, companies: [{ name: 'Example design studio', count: 18 }, { name: 'Example open-source group', count: 12 }], schools: [{ name: 'Example art school', count: 16 }, { name: 'Example institute of technology', count: 9 }] }; break;
    case '/api/v1/metrics/social': body = { as_of: '2026-09-18T12:00:00Z', scope: 'synthetic-demo', providers: { wechat: { ...stats, groups: 12, conversations: 64, indexed_messages: 4200, chat: { contacts: 64, messages: 4200, sent: 1900, received: 2300, leaders, group_leaders: leaders.map((r, i) => ({ ...r, name: `Interest group ${i + 1}`, value: 280 - i * 36 })) } }, instagram: { ...stats, people: 64, authors: 42, posts: 312, likes: 1800 }, linkedin: { ...stats, people: 48, authors: 24, posts: 186, charts: [{ title: 'Fields of work', denominator: 48, filter: 'company', rows: [{ name: 'Example design studio', profile_id: id(1), count: 18 }, { name: 'Example open-source group', profile_id: id(2), count: 12 }] }] } } }; break;

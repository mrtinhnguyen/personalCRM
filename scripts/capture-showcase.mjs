...
const context = await browser.newContext({ viewport: { width: 1440, height: 1080 }, deviceScaleFactor: 1, locale: 'zh-CN', timezoneId: 'UTC', serviceWorkers: 'block' });
const errors = [];
...
try {
  ...
  await page.getByRole('link', { name: '联系人 128', exact: true }).waitFor();
  ...
  for (const label of await page.locator('.profile-list-main strong').allTextContents()) {
    if (!/^联系人 \d{2}$/.test(label)) throw new Error('Unexpected contact label; refusing to capture.');
  }
  await capture('contacts');

  await open('/relationships');
  await page.locator('[data-graph-controls]:not([hidden])').waitFor();
  await page.getByRole('button', { name: '适合全部节点', exact: true }).click();

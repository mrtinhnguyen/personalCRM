...
const labels:Record<string,string>={wechat:"微信",instagram:"Instagram",linkedin:"LinkedIn",manual:"手工资料"};
export function PlacesMap({points}:{points:PlacePoint[]}){
 ...
 return <section className="places-map"><div className="map-filters"><label>年份<select value={year} onChange={e=>{requestedView.current="places";setYear(e.target.value);}}><option value="">全部</option>{years.map(y=><option key={y}>{y}</option>)}</select></label><label>来源<select value={source} onChange={e=>{requestedView.current="places";setSource(e.target.value);}}><option value="">全部平台</option>{[...new Set(points.flatMap(p=>p.sources||[]))].map(s=><option value={s} key={s}>{labels[s]||s}</option>)}</select></label><button className="button button-soft" onClick={()=>{requestedView.current="world";map.current?.setView([20,0],0);}}>查看全球</button><button className="button button-soft" onClick={()=>{requestedView.current="places";if(!map.current||!visible.length)return;import('leaflet').then(L=>map.current?.fitBounds(L.latLngBounds(visible.map(p=>[p.latitude,p.longitude])),{padding:[42,42],maxZoom:10}));}}>适合全部地点</button></div>
 <div className="atlas-map" ref={container} role="region" aria-label="社交足迹地图，可用加减按钮缩放到全球"/>
 {offline&&<p className="map-hint">详细底图暂时无法连接，正在显示本地世界轮廓。</p>}
 <div className="place-buttons">{visible.slice(0,12).map((p,i)=><button key={i} className={selected===p?'selected':''} onClick={()=>{requestedView.current=p;setSelected(p);map.current?.setView([p.latitude,p.longitude],10);}}>{p.label}<small>{p.count?`${p.count} 次`:'资料地址'}</small></button>)}</div>
 {selected&&<div className="place-detail" aria-live="polite"><strong>{selected.label}</strong><span>{selected.latitude.toFixed(5)}, {selected.longitude.toFixed(5)}</span><a href={`https://www.openstreetmap.org/?mlat=${selected.latitude}&mlon=${selected.longitude}#map=14/${selected.latitude}/${selected.longitude}`} target="_blank" rel="noreferrer">在 OpenStreetMap 打开</a>{(selected.events||[]).filter(e=>(!year||e.at.startsWith(year))&&(!source||e.source===source)).slice(0,12).map((e,i)=>e.url?<a key={i} href={e.url}>{e.at.slice(0,10)} · {e.text||'查看动态'}</a>:<span key={i}>{e.at.slice(0,10)} · {e.text||'已收录足迹'}</span>)}</div>}
 </section>;
}
